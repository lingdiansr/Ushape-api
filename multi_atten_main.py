...
class Trainer(object):
    ...
    def train(self, epoch, data_loader):
        ...
        for i, data_batch in tqdm(enumerate(data_loader), total=len(data_loader), desc=f"Epoch {epoch} Training"):
            ...
            if i % 1 == 0 and i > 0:
                label_result_temp = torch.cat(label_result)
                pred_result_temp = torch.cat(pred_result)
                p, r, f1, _ = precision_recall_fscore_support(label_result_temp.numpy(),
                                                              pred_result_temp.numpy(),
                                                              average="macro")
                tqdm.write(
                    f"Step {i}/{len(data_loader)} - Loss: {np.mean(loss_list):.4f}, F1: {f1:.4f}, Precision: {p:.4f}, Recall: {r:.4f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--predict_path', type=str, default='./Diakg_output.json')
