...
from multi_attention_unet import AttentionUNet
...
class IncreaseChannel(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(IncreaseChannel, self).__init__()
        # 1x1 卷积层，用于改变通道数
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.mlp=MLP(conv_input_size,config.unet_in_dim)
        self.segmentation_net = AttentionUNet(input_channels=config.unet_in_dim,
                                              class_number=config.unet_out_dim,
                                              down_channel=config.down_dim)

        self.liner = nn.Linear(config.lstm_hid_size, config.unet_in_dim)  # (768,3)
        self.channel_transform = nn.Linear(config.conv_hid_size * len(config.dilation), 3)
        self.increasechannels=IncreaseChannel(256,conv_input_size)
        # self.ner_classifier = nn.Linear(config.lstm_hid_size, config.ner_labels)
    ...
    def forward(self, bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length):
        ...
        # conv_outputs = torch.masked_fill(conv_outputs, grid_mask2d.eq(0).unsqueeze(-1), 0.0)#conv_outputs:Tensor(12,25,25,288)
        # outputs = self.predictor(word_reps, word_reps, conv_outputs)

        # feature_map = self.get_channel_map(sequence_output, entity_as)  # feature_map:Tensor(4,42,42,768)
        ##print('feature_map:', feature_map.shape)
        conv_outputs=self.mlp(conv_inputs)
        conv_outputs = conv_outputs.permute(0, 3, 1, 2)
        # conv_outputs = nn.Conv2d(conv_outputs.shape[1], 3, kernel_size=1)(conv_outputs)#Tensor(12,3,30,30)
        # attn_input = self.liner(conv_outputs).permute(0, 3, 1, 2).contiguous()  # attn_input:Tensor(4,3,42,42)
        # attn_input = conv_outputs.permute(0, 3, 1, 2).contiguous()
        attn_map = self.segmentation_net(conv_outputs)#Tensor(12,30,30,256)
        conv_inputs = torch.masked_fill(attn_map, grid_mask2d.eq(0).unsqueeze(-1),
                                        0.0)  # conv_inputs:Tensor(12,30,30,256)
        # print(conv_inputs.shape)
        conv_inputs = conv_inputs.permute(0, 3, 1, 2)
        conv_inputs=self.increasechannels(conv_inputs)
        conv_inputs = conv_inputs.permute(0, 2, 3, 1)
        conv_outputs = self.convLayer(conv_inputs)  # conv_outputs:Tensor(12,25,25,288)
        conv_outputs = torch.masked_fill(conv_outputs, grid_mask2d.eq(0).unsqueeze(-1),
                                         0.0)  # conv_outputs:Tensor(12,25,25,288)
        outputs = self.predictor(word_reps, word_reps, conv_outputs)  # outputs:Tensor(12,25,25,6)
        # ner_scores = self.ner_classifier(word_reps)
        return outputs
